...
def gameLoop():
    ...
    #draw apple
    snekCanvas.create_rectangle(applePoint[0], applePoint[1], applePoint[0] + dotSize , applePoint[1] + dotSize,
                                fill=appleColor, outline = appleBorderColor)

    # draw snake
    snekCanvas.create_rectangle(segments[0][0], segments[0][1], segments[0][0] + dotSize, segments[0][1] + dotSize,
                                fill="blue", outline="white")

    for i in range(1,dots):
        snekColor = '#%02x%02x%02x' % (random.randint(0,255), random.randint(0,255), random.randint(0,255))
